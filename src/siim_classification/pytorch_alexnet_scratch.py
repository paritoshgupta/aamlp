...
class AlexNet(nn.Module):
    ...
    def forward(self, image):
        ...
        bs, c, h, w = image.size()
        x = F.relu(self.conv1(image)) # size (bs, 96, 55, 55) ((w-f+2p)/s) + 1)
        x = self.pool1(x) # (bs, 96, 27, 27) ((w-f+2p)/s) + 1)
        x = F.relu(self.conv2(x)) # size (bs, 256, 27, 27
        x= self.pool2(x) # size (bs, 256, 13, 13)
        x = F.relu(self.conv3(image)) # size (bs, 384, 13, 13)
        x = F.relu(self.conv4(image)) # size (bs, 384, 13, 13)
        x = F.relu(self.conv5(image)) # size (bs, 256, 13, 13)
        x = self.pool3(x) # size (bs, 256, 6, 6)
        x = x.view(bs, -1) # size (bs, 9216)
        x = F.relu(self.fc1(x)) # size (bs, 4096)
        x = self.dropout1(x) # size (bs, 4096)
        x = self.fc2(x) # size (bs, 4096)
        x = self.dropout2(x) # size (bs, 4096)
        x = F.relu(self.fc3(x)) # size (bs, 1000)
        # 1000 is number of classes in ImageNet dataset
        # softmax is an activation function that converts linear output to praobabilities that add upto 1
